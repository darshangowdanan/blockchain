        ...
        >
          ...
          <div className="flex flex-col w-full md:w-1/4 relative">
            ...
            {fromSuggestions.length > 0 &&
              renderSuggestions(fromSuggestions, (s) => {
                setFrom(s.stop_name);
                setFromId(s.stop_id);
                setFromSuggestions([]);
              })}
          ...
          <div className="flex flex-col w-full md:w-1/4 relative">
            ...
            {toSuggestions.length > 0 &&
              renderSuggestions(toSuggestions, (s) => {
                setTo(s.stop_name);
                setToId(s.stop_id);
                setToSuggestions([]);
              })}
          ...
          <div className="flex flex-col w-full md:w-1/6">
            ...
            <div className="flex items-center justify-center gap-2 bg-white/10 px-3 py-2 rounded-lg">
              ...
              <button
                type="button"
                onClick={() =>
                  setSelectedTickets(Math.min(15, selectedTickets + 1))
                }
                className="rounded bg-white/10 px-3 py-1 text-sm hover:bg-white/20"
              >
                +
            ...
            {journeys.map((j, idx) => (
              ...
              >
                ...
                <span className="font-semibold text-white">
                  {j.passengers} Passenger
                  {j.passengers > 1 ? "s" : ""} - ₹{j.amount}
                </span>
              </div>
            ))}
